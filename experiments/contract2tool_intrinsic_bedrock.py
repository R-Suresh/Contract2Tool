...
tool contracts from evidence views, normalizes predictions, and scores them
against gold contracts.
...
def normalize_vars(values: Any, vocab: Set[str]) -> Tuple[List[str], List[str]]:
    ...
    for item in raw:
        ...
        # Simple fuzzy fallback: exact after singular/plural cleanup.
...
def build_synthetic_doc(tool: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"Use this tool only when its operation matches the current workflow step."
    )
...
def invoke_bedrock(client, model_id: str, prompt: str, max_tokens: int = 700) -> Tuple[str, Dict[str, Any]]:
    ...
    try:
        ...
        # Some models/providers reject temperature. Retry without it.
        msg = str(e)
...
def aggregate(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    out = []
    metric_keys = [
        "requires_precision", "requires_recall", "requires_f1",
        "produces_precision", "produces_recall", "produces_f1",
        "risk_accuracy", "cost_accuracy", "exact_match",
        "valid_json", "normalization_ok",
    ]
...
def main():
    ...
    parser.add_argument("--module_path", required=True, help="Path to scaledExperiment_main.py or module containing TOOLS")
